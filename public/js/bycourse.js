...
function make_buttons(processed,sort_method) {
  new_processed = [];
  ...
  for(var i = 0; i < new_processed.length; i++){
    if(new_processed[i].name.length > 0) {
      make_button(new_processed[i].name);
    }
  }
}
...
function computeStats(){
  var departments = {};
  d3.csv("bycourse/courses.csv", function(error, data){
...
//Work on figure 2 will start below